...
import {
  LayoutDashboard, Pill, Activity, Apple, MessageSquare,
  Calendar, Bot, UserCircle, HeartPulse, ChevronLeft, ChevronRight, LogOut,
} from "lucide-react";
...
export default function Sidebar() {
  ...
  const roleLabel   = ROLE_LABELS[profile?.role ?? "patient"];
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className={cn("flex items-center gap-2.5 border-b border-border transition-all duration-300", collapsed ? "p-4 justify-center" : "p-4 px-5")}>
        ...
        {!collapsed && (
          <div className="overflow-hidden">
            <h1 className="font-bold text-base tracking-tight leading-tight text-text-primary">RenaCare</h1>
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive ? "bg-navy-50 text-navy-700" : "text-text-secondary hover:bg-surface-muted hover:text-text-primary"
              )}
            >
              <item.icon size={18} strokeWidth={isActive ? 2.5 : 2} className={cn("flex-shrink-0 transition-colors", isActive ? "text-navy-600" : "text-text-tertiary")} />
              {!collapsed && <span>{item.label}</span>}
              {isActive && !collapsed && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 bg-navy-500 rounded-r-full" />}
            </Link>
          );
        })}
      </nav>
      ...
      <div className={cn("border-t border-border pt-3 pb-3 space-y-2", collapsed ? "px-2" : "px-3")}>
        <div className={cn("flex items-center gap-2.5 rounded-[var(--radius-lg)] bg-surface-muted transition-all", collapsed ? "p-2 justify-center" : "p-2.5 px-3")}>
          ...
          {!collapsed && (
            <div className="min-w-0">
              <p className="text-sm font-medium text-text-primary truncate leading-tight">{shortName}</p>
              <p className="text-[11px] text-text-tertiary leading-tight">{roleLabel}</p>
            </div>
          )}
        </div>
