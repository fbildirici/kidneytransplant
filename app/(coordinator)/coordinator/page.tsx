...
export default function CoordinatorPage() {
  ...
  return (
    <>
      <PageTitle title="Koordinatör Paneli" />
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <CalendarCheck2 size={15} className="text-navy-200" />
              <span className="text-navy-200 text-sm font-semibold">Koordinatör Onay Merkezi</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1.5">Randevu Onayları</h1>
            <p className="text-white/70 text-sm">Doktor ve diyetisyen için oluşturulan bekleyen randevu taleplerini yönetin.</p>
          </div>
          <div className="rounded-[var(--radius-lg)] bg-white/10 border border-white/15 px-4 py-3 text-center">
      ...
      <div className="rounded-[var(--radius-xl)] border border-border bg-surface p-5 shadow-card">
        <div className="flex items-center gap-2 mb-4">
          ...
          <p className="text-sm font-bold text-text-primary">Bekleyen Onay Listesi</p>
        </div>

        <div className="space-y-3">
          {pending.length === 0 ? (
            <p className="text-sm text-text-muted">Bekleyen randevu talebi kalmadı.</p>
          ) : (
            pending.map((slot) => (
              <div key={slot.id} className="rounded-[var(--radius-xl)] border border-border bg-surface-muted p-4">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="rounded-[var(--radius-lg)] bg-rose-500 px-4 py-2 text-sm font-semibold text-white hover:bg-rose-600 cursor-pointer"
                    >
                      <span className="flex items-center gap-2">
                        <XCircle size={15} />
                        Reddet
